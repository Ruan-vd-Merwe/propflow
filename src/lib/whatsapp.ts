...
function normaliseSaPhone(raw: string): string | null {
  ...
  if (digits.startsWith('0')  && digits.length === 10) return '27' + digits.slice(1)  // 0XX → 27XX
...
export function sendPaymentReminder(opts: {
  phone: string; name: string; amount: number; property: string; dueDate: string
}) {
  return sendWhatsApp(
    opts.phone,
...
export function sendPaymentOverdue(opts: {
  phone: string; name: string; amount: number; property: string; daysLate: number
}) {
  return sendWhatsApp(
    opts.phone,
...
export function sendMaintenanceUpdate(opts: {
  phone: string; name: string; subject: string; status: string
}) {
  return sendWhatsApp(
    opts.phone,
...
export function sendIntroductionWhatsApp(opts: {
  phone: string; name: string; suburb: string
}) {
  return sendWhatsApp(
    opts.phone,
...
export function sendTenantPaidNotification(opts: {
  landlordPhone: string; tenantName: string; amount: number; property: string
}) {
  return sendWhatsApp(
    opts.landlordPhone,
...
export function sendBookingToProvider(opts: {
  phone: string; providerName: string; service: string;
  tenantName: string; property: string; date: string; notes?: string
}) {
