...
export function calcRatioFlag(
  ...
  const flag: RatioFlag = percent < 25 ? 'green' : percent <= 33 ? 'amber' : 'red'
...
export function detectFraudFlags(inputs: FraudCheckInputs): string[] {
  ...
  if (
    ...
    bankAnalysis.salaryMonths < Math.floor(bankAnalysis.totalMonthsAnalyzed * 0.83)
  ) {
...
export function calcCreditScore(inputs: CreditScoreInputs): {
  ...
  const { bankAnalysis, rentCents, incomeCents, idVerification, fraudFlags, referenceChecks } = inputs
  ...
  const bankHealthScore = Math.min(40, incomePoints + bouncedPoints + gamblingPoints)
  ...
  if (rentCents > 0 && incomeCents > 0) {
    ...
    ratioScore   = result.flag === 'green' ? 25 : result.flag === 'amber' ? 15 : 5
  }

  // ── 3. ID verification (15 pts) ─────────────────────────────────────────────
  const checksumPoints    = idVerification.checksumValid ? 8 : 0
  const idFraudFlags      = fraudFlags.filter((f) => f === 'DOB_MISMATCH' || f === 'DUPLICATE_ID_IN_SYSTEM')
  ...
  const responseRank: Record<ReferenceResponse, number> = {
    positive: 3, neutral: 2, no_response: 1, negative: 0,
  }
  ...
  if (referenceChecks.length > 0) {
    ...
    if (contacted.length > 0) {
      bestResponse = contacted.reduce<ReferenceResponse>((best, r) =>
        responseRank[r.response] > responseRank[best] ? r.response : best,
        contacted[0].response
      )
      refScore =
        bestResponse === 'positive'    ? 20 :
        bestResponse === 'neutral'     ? 12 :
        bestResponse === 'no_response' ? 10 : 0
    }
  }

  // ── Total ────────────────────────────────────────────────────────────────────
  const total = Math.max(0, Math.min(100,
    bankHealthScore + ratioScore + idScore + refScore
  ))
