...
const SA_LOCATIONS = [
  'Cape Town', 'Sea Point', 'Green Point', 'Gardens',
  'Woodstock', 'Bellville', 'Durbanville', 'Stellenbosch',
  'Paarl', 'Somerset West', 'Constantia', 'Claremont',
  'Rondebosch', 'Observatory', 'Johannesburg', 'Sandton',
  'Pretoria', 'Durban', 'Umhlanga', 'Port Elizabeth',
  'Gqeberha', 'Bloemfontein', 'East London',
...
function detectCategory(
  ...
  if (text.includes('interest rate') || text.includes('repo rate') ||
      text.includes('bond') || text.includes('afford'))
    return 'Interest rates'

  if (text.includes('rent') || text.includes('tenant') || text.includes('lease'))
    return 'Rental market'

  if (text.includes('price') || text.includes('value') ||
      text.includes('growth') || text.includes('decline'))
    return 'Property prices'

  if (text.includes('develop') || text.includes('new unit') ||
      text.includes('launch') || text.includes('complex'))
    return 'New developments'

  if (text.includes('rezone') || text.includes('rezoning') ||
      text.includes('zoning') || text.includes('planning'))
    return 'Rezoning and planning'

  if (text.includes('cape town') || text.includes('western cape') ||
      text.includes('sea point') || text.includes('stellenbosch'))
    return 'Cape Town property news'

  if (text.includes('invest') || text.includes('yield') ||
      text.includes('return') || text.includes('portfolio'))
  ...
  if (hint) {
    const match = CATEGORIES.find(c => c.toLowerCase() === hint.toLowerCase())
...
function scoreRelevance(title: string, excerpt: string): number {
  ...
  const highValue = [
    'cape town', 'south africa', 'rental', 'property',
    'interest rate', 'landlord', 'tenant', 'price',
    'market', 'development', 'afford',
  ]
...
export async function fetchAllSources(): Promise<{
  ...
  if (!sources?.length) return { fetched: 0, stored: 0, errors: ['No active sources'] }
  ...
  for (const source of sources) {
    ...
    try {
      ...
      for (const item of feed.items ?? []) {
        ...
        const category   = detectCategory(item.title, excerpt, source.category_hint)
