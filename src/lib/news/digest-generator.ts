...
export async function generateWeeklyDigest(): Promise<{
  ...
  // AI intro
  let introText = 'Here is your weekly South African property market update from PropTrust.'
    ...
    try {
      ...
      const msg = await client.messages.create({
        ...
        messages: [{
          role: 'user',
          content:
            `Write a 2-3 sentence intro for a weekly South African property newsletter based on these top headlines:\n${titles}\n\nTone: professional, clear, useful. No hype. No emojis. Plain text only.`,
        }],
      })
      if (msg.content[0].type === 'text') introText = msg.content[0].text.trim()
    } catch (e) {
  ...
  if (error ?? !digest) throw new Error(`Failed to store digest: ${error?.message}`)
...
function buildDigestHTML(
  ...
  const dateRange =
    weekStart.toLocaleDateString('en-ZA', { day: 'numeric', month: 'long' }) +
    ' – ' +
    weekEnd.toLocaleDateString('en-ZA', { day: 'numeric', month: 'long', year: 'numeric' })
...
function buildDigestText(
  ...
  lines.push('---')
  lines.push('Unsubscribe: https://proptrust.co.za/unsubscribe')
  return lines.join('\n')
}
