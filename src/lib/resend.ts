...
function buildPaymentEmail(
  ...
  switch (type) {
    case "payment_before_3d":
      return paymentReminderEmail(data);
    case "payment_due_today":
      return paymentDueTodayEmail(data);
    case "payment_late_3d":
      return paymentLate3dEmail(data);
    case "payment_late_7d":
      return paymentLate7dEmail(data);
    case "payment_late_14d":
      return paymentLate14dEmail(data);
    default:
...
export async function sendIntroductionEmails(opts: {
  ...
  const tResult: SendResult =
    tRes.status === "fulfilled" && !tRes.value.error
      ...
      : {
          ...
          error:
            tRes.status === "rejected"
              ? String(tRes.reason)
              : tRes.value.error?.message,
        };

  const lResult: SendResult =
    lRes.status === "fulfilled" && !lRes.value.error
