...
export function affordability_score(profile: TenantInterestProfile, property: PropertyData): number {
  ...
  const income_score =
    income_ratio <= 0.25 ? 1.0 :
    income_ratio <= 0.30 ? 0.85 :
    income_ratio <= 0.35 ? 0.65 :
    income_ratio <= 0.40 ? 0.45 :
    income_ratio <= 0.50 ? 0.25 : 0.05

  const budget_score =
    budget_ratio <= 0.85 ? 1.0 :
    budget_ratio <= 1.00 ? 0.80 :
    budget_ratio <= 1.10 ? 0.50 :
    budget_ratio <= 1.25 ? 0.25 : 0.05

  return weighted_sum([[income_score, 0.55], [budget_score, 0.45]])
}

export function proximity_score(profile: TenantInterestProfile, property: PropertyData): number {
...
export function soft_preference_score(interests: string[], tags: string[]): number {
  ...
  for (const interest of interestsLower) {
    const best = tagsLower.reduce((max, t) => {
      if (t.includes(interest) || interest.includes(t)) return Math.max(max, 1.0)
      const shorter = interest.length < t.length ? interest : t
      const longer  = interest.length < t.length ? t : interest
      if (longer.includes(shorter.slice(0, Math.max(3, shorter.length - 2)))) return Math.max(max, 0.5)
      return max
    }, 0)
    score += best
  }
...
export function tenant_affordability_insight(
  profile: TenantInterestProfile,
  property: PropertyData,
): { score: number; message: string; income_ratio: number; budget_ratio: number } {
  const rent = property.rent ?? 0
  ...
  const total_extras = elec + water + internet + (property.tenant_extra_costs ?? 0)
...
export function property_fit_insight(
  ...
  // Bedroom match
  if (profile.desired_bedrooms !== undefined && property.bedrooms !== undefined) {
    const diff = Math.abs(property.bedrooms - profile.desired_bedrooms)
    const bed_score = diff === 0 ? 1.0 : diff === 1 ? 0.65 : 0.30
    scores.push([bed_score, 0.35])
    if (diff === 0) notes.push(`${property.bedrooms}-bed matches your preference.`)
    else if (diff === 1) notes.push(`${property.bedrooms}-bed — one off your preferred ${profile.desired_bedrooms}.`)
  } else {
  ...
  if (profile.has_car) {
    ...
    if (!property.parking_available) notes.push('No parking listed — may need street parking.')
  } else {
  ...
  // Pets
  if (profile.has_pets) {
    scores.push([property.pets_allowed ? 1.0 : 0.0, 0.10])
  } else {
    scores.push([0.8, 0.10])
  }
...
export function area_fit_insight(
  ...
  scores.push([normalize(school, 0, 10), 0.10])

  // Retail access
  const retail = property.retail_access_score ?? 5
  scores.push([normalize(retail, 0, 10), 0.10])
  ...
  const preferred = profile.preferred_suburbs ?? []
  const inArea = preferred.length > 0 && property.suburb &&
...
export function lifestyle_fit_insight(
  ...
  if (property.minutes_to_grocery !== undefined)
    proximity_scores.push(inverse_normalize(property.minutes_to_grocery, 0, 20))
  ...
  if (property.minutes_to_restaurant !== undefined)
    proximity_scores.push(inverse_normalize(property.minutes_to_restaurant, 0, 15))

  const avg_proximity = proximity_scores.length > 0
  ...
  const social_boost = interestsLower.some((i) => ['social', 'nightlife', 'bars', 'restaurants'].includes(i))
    ? normalize(property.social_life_score ?? 5, 0, 10)
    : 0.6
  scores.push([social_boost, 0.15])

  const outdoor_boost = interestsLower.some((i) => ['running', 'hiking', 'cycling', 'outdoor', 'nature'].includes(i))
    ? normalize(property.outdoor_lifestyle_score ?? 5, 0, 10)
    : 0.6
  scores.push([outdoor_boost, 0.15])

  const remote_boost = interestsLower.some((i) => ['remote', 'work from home', 'wfh', 'freelance'].includes(i))
...
export function commute_fit_insight(
  ...
  if (work_locations.length === 0) {
    ...
    const walk = property.walkability_score ?? 5
    const score = has_car ? 0.7 : weighted_sum([[normalize(transport, 0, 10), 0.6], [normalize(walk, 0, 10), 0.4]])
    return {
      score,
      message: 'No work locations saved. Add them to your profile for a commute score.',
    }
  }
  ...
  if (!has_car) {
    ...
    score = weighted_sum([[score, 0.6], [normalize(transit, 0, 10), 0.4]])
  }

  const message =
    avg <= 15 ? `Excellent commute — ${Math.round(avg)} min average to work.` :
    avg <= 30 ? `Good commute — ${Math.round(avg)} min to work.` :
    avg <= 45 ? `Moderate commute — ${Math.round(avg)} min to work.` :
    avg <= 60 ? `Long commute — ${Math.round(avg)} min. Consider transport costs.` :
...
export function tenant_deal_quality_insight(
  property: PropertyData,
): { score: number; message: string; pct_vs_avg?: number } {
  const rent = property.rent ?? 0
...
export function tenant_safety_insight(
  property: PropertyData,
): { score: number; message: string } {
  ...
  const message =
    score >= 0.80 ? 'Safe area with low crime and good security presence.' :
    score >= 0.60 ? 'Reasonably safe area with average security.' :
    score >= 0.40 ? 'Moderate safety concerns — check local security.' :
...
export function tenant_approval_insight(
  ...
  const income_score =
    ...
    income_ratio <= 0.30 ? 0.85 :
    income_ratio <= 0.35 ? 0.65 :
    income_ratio <= 0.40 ? 0.40 : 0.20

  const competition_score = inverse_normalize(apps, 0, 20)

  const score = weighted_sum([[income_score, 0.70], [competition_score, 0.30]])
  const probability_pct = Math.round(score * 100)

  const message =
    score >= 0.75 ? `Strong approval profile — ${probability_pct}% estimated likelihood.` :
    score >= 0.55 ? `Good approval chances — ${probability_pct}%. ${apps} competing applications.` :
    score >= 0.40 ? `Moderate chances — ${probability_pct}%. ${apps} other applications.` :
...
export function tenant_match_confidence(property: PropertyData): number {
  const checks: boolean[] = [
    ...
    (property.commute_times !== undefined && Object.keys(property.commute_times).length > 0),
...
export function generate_interest_match_reasons(
  ...
  const reasons: string[] = []
  const order = ['affordability', 'area_fit', 'property_fit', 'lifestyle_fit', 'commute', 'deal_quality', 'safety']
...
export function generate_interest_match_warnings(
  ...
  if (insights.affordability?.score !== undefined && insights.affordability.score < 0.40) {
    warnings.push(insights.affordability.message)
  }
  if (insights.safety?.score !== undefined && insights.safety.score < 0.40) {
  ...
  if (insights.deal_quality?.score !== undefined && insights.deal_quality.score < 0.35) {
...
export function tenant_interest_match_model(
  ...
  const warnings = generate_interest_match_warnings(profile, property, insights)
