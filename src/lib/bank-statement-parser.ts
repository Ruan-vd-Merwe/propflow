...
const BANK_PATTERNS: { name: string; patterns: RegExp[] }[] = [
  { name: 'FNB',          patterns: [/first national bank/i, /\bfnb\b/i, /rmb private/i] },
...
const SALARY_KW: RegExp[] = [
  /\bsalary\b/i, /\bsal\s/i, /\bpayroll\b/i, /\bwages\b/i,
  /\bremuneration\b/i, /net pay/i, /monthly pay/i, /\bpay\s+run\b/i,
  /employer.*pay/i, /payslip/i, /\bincome deposit\b/i,
]

const BOUNCED_KW: RegExp[] = [
  /return.*debit/i, /debit.*return/i, /\brd\b.*fee/i,
  /dishonour/i, /unpaid.*do\b/i, /do.*unpaid/i,
  /failed.*debit/i, /unsuccessful.*debit/i, /\bdo return\b/i,
  /\bcharge.?back\b/i, /returned.*debit.*order/i, /debit.*order.*return/i,
  /\bunpaid debit order\b/i,
]

const GAMBLING_KW: RegExp[] = [
  /hollywoodbets/i, /\bhollywd\b/i,
  /sportingbet/i, /sports.?betting/i,
  /sun international/i, /\bsunbet\b/i,
  /\bbetway\b/i, /\bbet365\b/i,
  /world sports betting/i, /\bwsb bet\b/i,
  /lottostar/i, /\bnational lottery\b/i, /lucky.?numbers/i,
  /\bsportpesa\b/i, /\bdraftking/i,
]

const RENTAL_KW: RegExp[] = [
  /\brent\b/i, /\brental\b/i, /\blandlord\b/i,
  /\blease\s+pay/i, /\btenant\b/i, /accommodation.*pay/i,
]
...
export function parseBankStatementText(rawText: string): BankStatementAnalysis {
  ...
  for (const { name, patterns } of BANK_PATTERNS) {
    if (patterns.some((p) => p.test(rawText))) { bank = name; break }
  }
  if (!bank) warnings.push('Could not identify the bank from the document text.')
  ...
  const gamblingTransactions = transactions.filter((t) => t.type === 'gambling')
  // Rental payments going OUT (debit)
  const rentalPayments       = transactions.filter((t) => t.type === 'rental' && t.amount < 0)
  ...
  const avgMonthlyIncome   = n > 0
    ? Math.round(monthlyBreakdowns.reduce((s, m) => s + m.income,         0) / n) : 0
  const avgMonthlyExpenses = n > 0
    ? Math.round(monthlyBreakdowns.reduce((s, m) => s + m.expenses,       0) / n) : 0
  const avgMonthlyBalance  = n > 0
    ? Math.round(monthlyBreakdowns.reduce((s, m) => s + m.closingBalance, 0) / n) : 0
 ...
 */
function extractTransactions(text: string, warnings: string[]): BankTransaction[] {
  const txns: BankTransaction[] = []
  const seen = new Set<string>()

  function add(date: string, desc: string, amountStr: string, balanceStr: string) {
    const trimDesc = desc.replace(/\s{2,}/g, ' ').trim()
    if (trimDesc.length < 3) return
    if (/^(date|description|balance|amount|debit|credit|details|reference)$/i.test(trimDesc)) return
    ...
    txns.push({ date, description: trimDesc, amount: cents, balance, type, flags })
  }
...
function groupByMonth(txns: BankTransaction[]): MonthlyBreakdown[] {
  ...
  map.forEach((list: BankTransaction[], month: string) => {
    ...
    const income   = list.filter((t: BankTransaction) => t.amount > 0)
                         .reduce((s: number, t: BankTransaction) => s + t.amount, 0)
    const expenses = Math.abs(
                       list.filter((t: BankTransaction) => t.amount < 0)
                           .reduce((s: number, t: BankTransaction) => s + t.amount, 0)
                     )
    ...
    result.push({
      ...
      bouncedDoCount: list.filter((t: BankTransaction) => t.type === 'bounced_do').length,
      gamblingCount:  list.filter((t: BankTransaction) => t.type === 'gambling').length,
    })
  })
