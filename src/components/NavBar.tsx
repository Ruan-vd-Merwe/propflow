...
export function NavBar() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (!user) return
      supabase.from('profiles').select('is_landlord, is_tenant').eq('id', user.id).single()
        .then(({ data }) => { if (data) setRoles(data as UserRoles) })
    })
          ...
          {isDual && (
            <>
              <li className="bg-blue-50 px-6 py-2">
                <p className="text-xs font-semibold uppercase tracking-wider text-blue-400">Tenant</p>
              </li>
