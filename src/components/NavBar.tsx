...
function getPrimaryLinks(roles: UserRoles, pathname: string): NavLink[] {
  if (isInSection(pathname, "/tenant") && roles.is_tenant)
...
export function NavBar() {
  ...
  const [roles, setRoles] = useState<UserRoles>({
    is_landlord:
      !isInSection(pathname, "/connector") && !isInSection(pathname, "/tenant"),
  ...
  const isTenantNav = primaryLinks.some(
    (link) => link.href === "/tenant/dashboard",
  );
  const tenantMainLinks = isTenantNav
    ? primaryLinks.filter((link) => link.href !== "/contact")
    : primaryLinks;
  const tenantLivingLinks = MORE_ACCOUNT.filter(
    (link) => link.href === "/services" || link.href === "/neighbour",
  );
  ...
  const guideRole = isTenantNav ? "tenant" : "landlord";
  const canShowGuide =
    activeSurface !== "connector" && (roles.is_tenant || roles.is_landlord);
  const showDesktopGuide =
    canShowGuide &&
    pathname !== "/dashboard" &&
      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:px-6 sm:py-3.5">
          ...
          <div ref={moreRef} className="relative ml-0.5">
            ...
            {moreOpen && (
              <div className="absolute right-0 top-full z-50 mt-1.5 w-64 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-lg">
                ...
                {showDesktopGuide && (
                  <DropdownSection label="PropTrust Guide">
                    <PropTrustGuide role={guideRole} variant="menu" />
                  </DropdownSection>
                )}
              </div>
            )}
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="flex h-11 w-11 items-center justify-center rounded-lg text-slate-600 transition hover:bg-slate-100 sm:hidden"
          >
            {menuOpen ? (
              <svg
                className="h-5 w-5"
          ...
          <ul className="divide-y divide-slate-100">
            ...
            {isTenantNav ? (
              <>
                ...
                {canShowGuide && (
                  <li>
                    <PropTrustGuide role="tenant" variant="menu" />
                  </li>
                )}
              ...
              <>
                ...
                {canShowGuide && (
                  <li>
                    <PropTrustGuide role="landlord" variant="menu" />
                  </li>
                )}
...
function DropdownLink({
  ...
  return (
    ...
    >
      <Icon
        className={`h-4 w-4 shrink-0 ${active ? "text-slate-700" : "text-slate-400"}`}
      />
      {label}
    </Link>
  );
