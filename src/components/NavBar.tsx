...
export function NavBar() {
  ...
  useEffect(() => {
    ...
    function onOutside(e: PointerEvent) {
      ...
      if (navRef.current?.contains(target) || menuOverlayRef.current?.contains(target)) return;
      setMenuOpen(false);
    }
      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:px-6 sm:py-3.5">
        ...
        <div className="hidden items-center gap-0.5 sm:flex">
          ...
          <div ref={moreRef} className="relative ml-0.5">
            ...
            >
              More
              <IconChevronDown className={`h-3.5 w-3.5 transition-transform duration-150 ${moreOpen ? "rotate-180" : ""}`} />
            </button>
                ...
                {roles.is_landlord && (
                  <DropdownSection label="Property Management">
                    {MORE_PROPERTY_MGMT.map(({ href, label, Icon }) => (
                      <DropdownLink key={href} href={href} label={label} Icon={Icon} active={isActive(href)} onClose={() => setMoreOpen(false)} />
                    ))}
                  </DropdownSection>
                )}
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-lg text-slate-900 transition hover:bg-slate-100 sm:hidden"
          >
            {menuOpen ? (
                ...
                {availableSurfaces.map(({ surface, label, href }) => (
                  <Link
                    ...
                    className={`rounded-full px-3 py-1.5 text-xs font-medium transition ${
                      activeSurface === surface ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-500"
                    }`}
                  >
                    {label}
                  </Link>
            ...
            {roles.is_landlord && (
              <>
                <li className="bg-slate-50 px-6 py-2">
                  <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Property Management</p>
                </li>
                {MORE_PROPERTY_MGMT.map(({ href, label, Icon }) => {
                  const active = isActive(href);
                  return (
            ...
            {roles.is_tenant && (
              <>
                <li className="bg-blue-50 px-6 py-2">
                  <p className="text-xs font-semibold uppercase tracking-wider text-blue-400">Rental Search</p>
                </li>
                {MORE_TENANT_SEARCH.map(({ href, label, Icon }) => {
                  const active = isActive(href);
                  return (
            ...
            {roles.is_connector && (
              <>
                <li className="bg-amber-50 px-6 py-2">
                  <p className="text-xs font-semibold uppercase tracking-wider text-amber-500">Connector</p>
                </li>
                {MORE_CONNECTOR_TASKS.map(({ href, label, Icon }) => {
                  const active = isActive(href);
                  return (
