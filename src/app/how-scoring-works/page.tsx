      ...
      <section className="bg-[#0f172a] px-6 pb-16 pt-14 text-center">
        <div className="mx-auto max-w-3xl">
          ...
          <p className="mx-auto mt-5 max-w-xl text-lg leading-relaxed text-slate-400">
            Our matching engine analyses 8 dimensions to connect the right tenants with
      ...
      <main className="mx-auto max-w-4xl px-4 py-14 sm:px-6">
        ...
        <section className="mb-16">
          ...
          <p className="mb-8 text-base text-slate-500">
            Every listed property is scored across 8 dimensions. The overall match score
            is a weighted average — dimensions you care about most count more.
          </p>
            ...
            <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1">
              {DIMENSIONS.map((d) => (
                <span key={d.name} className="text-xs text-slate-400">
                  {d.name}{' '}
                  <span className="font-semibold text-slate-200">{d.weight}</span>
                </span>
              ))}
            </div>
          </div>
        ...
        <section className="mb-16">
          ...
          <p className="mb-6 text-base text-slate-500">
            Each score has a confidence percentage. Scores based on more complete data
            are more reliable. A confidence below 40% means the score is indicative only.
          </p>
              ...
              <tbody className="divide-y divide-slate-100">
                {CONFIDENCE_TABLE.map((row) => (
                  <tr key={row.data} className="bg-white">
                    <td className="px-5 py-3 font-medium text-slate-700">{row.data}</td>
        ...
        <section className="mb-12 rounded-2xl bg-[#0f172a] p-8 text-white">
          <p className="text-base font-semibold">A note on how to use these scores</p>
          <p className="mt-3 text-sm leading-relaxed text-slate-300">
            PropTrust matching scores are designed to help you prioritise properties worth
            viewing. They are not a guarantee of approval or financial advice. Always do your
            own due diligence before signing a lease.
          </p>
        </section>
