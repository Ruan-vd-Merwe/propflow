      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Manage lease agreements and legal protection</p>
          </div>
          <Link
            href="/leases/new"
        ...
        {rows.length === 0 ? (
          <div className="card p-16 text-center">
            ...
            <p className="mt-4 font-semibold text-slate-700">No lease agreements yet</p>
            <p className="mt-1 text-sm text-slate-400">Create your first lease to get started.</p>
              ...
              {rows.map((lease) => (
                <div key={lease.id}
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 truncate">{lease.properties?.address}</p>
                  </div>
                  <p className="text-sm text-slate-700">{lease.tenants?.full_name ?? '—'}</p>
                  <p className="text-sm text-slate-600">{fmtDate(lease.lease_start)}</p>
                  <p className="text-sm font-medium text-slate-900">{fmtRand(lease.monthly_rent)}</p>
