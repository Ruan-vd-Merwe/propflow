...
type Tenant   = { id: string; property_id: string; full_name: string; email: string; monthly_rent: number }
  ...
  return (
    <div className="mb-8 flex items-center gap-0">
      {STEP_LABELS.map((label, i) => {
        ...
        return (
          <div key={n} className={`flex flex-1 items-center ${i < STEP_LABELS.length - 1 ? 'gap-0' : ''}`}>
            <div className="flex flex-col items-center gap-1">
              ...
              <span className={`hidden text-xs font-medium sm:block ${current ? 'text-slate-900' : 'text-slate-400'}`}>
                {label}
              </span>
            </div>
            {i < STEP_LABELS.length - 1 && (
              <div className={`mx-2 h-0.5 flex-1 ${done ? 'bg-emerald-400' : 'bg-slate-200'}`} />
            )}
          </div>
        )
      })}
...
const INPUT = 'w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm focus:border-blue-400 focus:outline-none'
...
export function LeaseWizard({ properties, tenants, landlord }: Props) {
  ...
  async function generate() {
    ...
    try {
      ...
      if (!res.ok) { setError(json.error ?? 'Failed to create lease'); return }
  ...
  const step2Valid = !!leaseStart && !!monthlyRent && parseFloat(monthlyRent) > 0
    ...
    <div className="mx-auto max-w-2xl">
      ...
      {step === 1 && (
        <div className="card p-6">
          <h2 className="mb-6 text-lg font-semibold text-slate-900">Select property and tenant</h2>
          <div className="space-y-5">
            <Field label="Property" required>
              <select className={SELECT} value={propertyId} onChange={(e) => onPropertyChange(e.target.value)}>
            ...
            <Field label="Tenant" required>
              <select className={SELECT} value={tenantId} onChange={(e) => onTenantChange(e.target.value)} disabled={!propertyId}>
                <option value="">{propertyId ? 'Select a tenant…' : 'Select a property first'}</option>
                {propertyTenants.map((t) => (
                  <option key={t.id} value={t.id}>{t.full_name}</option>
                ))}
              </select>
              {propertyId && propertyTenants.length === 0 && (
                <p className="mt-1.5 text-xs text-amber-600">No tenants on this property.</p>
              )}
            </Field>

            {selectedProperty && selectedTenant && (
              <div className="rounded-xl border border-slate-100 bg-slate-50 p-4 space-y-2">
                <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Auto-filled details</p>
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    <p className="text-xs text-slate-400">Landlord</p>
                    <p className="font-medium text-slate-900">{landlord.full_name}</p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-400">Tenant</p>
                    <p className="font-medium text-slate-900">{selectedTenant.full_name}</p>
                  </div>
      ...
      {step === 2 && (
        <div className="card p-6">
          <h2 className="mb-6 text-lg font-semibold text-slate-900">Lease terms</h2>
          <div className="space-y-5">
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              <div>
                <div className="mb-2 flex items-center justify-between">
                  <label className="text-sm font-medium text-slate-700">Lease end date</label>
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <Field label="Payment due day">
                <select className={SELECT} value={paymentDueDay} onChange={(e) => setPaymentDueDay(e.target.value)}>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="flex items-center justify-between rounded-xl border border-slate-200 p-3">
                ...
                >
                  <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition ${petAllowed ? 'translate-x-6' : 'translate-x-1'}`} />
                </button>
              </div>
              <div className="flex items-center justify-between rounded-xl border border-slate-200 p-3">
                <span className="text-sm font-medium text-slate-700">Subletting allowed</span>
                <button
                  type="button"
                ...
                >
                  <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition ${sublettingAllowed ? 'translate-x-6' : 'translate-x-1'}`} />
                </button>
              </div>
            </div>
          ...
          <div className="mt-8 flex justify-between">
            <button onClick={() => setStep(1)} className="rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-medium text-slate-600 transition hover:bg-slate-50">
              ← Back
            </button>
            <button
              disabled={!step2Valid}
      ...
      {step === 3 && selectedProperty && selectedTenant && (
        <div className="card p-6">
          <h2 className="mb-6 text-lg font-semibold text-slate-900">Review your lease</h2>
          ...
          <div className="space-y-4 text-sm">
            ...
            <Section title="Lease Terms">
              <Row label="Start date" value={new Date(leaseStart).toLocaleDateString('en-ZA', { day: 'numeric', month: 'long', year: 'numeric' })} />
              <Row label="End date" value={monthToMonth ? 'Month-to-month' : leaseEnd ? new Date(leaseEnd).toLocaleDateString('en-ZA', { day: 'numeric', month: 'long', year: 'numeric' }) : '—'} />
              <Row label="Monthly rent" value={fmtRand(monthlyRent)} />
              <Row label="Deposit" value={depositAmount ? fmtRand(depositAmount) : `${fmtRand(monthlyRent)} × 2 (default)`} />
              <Row label="Payment due" value={`${paymentDueDay}${parseInt(paymentDueDay) === 1 ? 'st' : parseInt(paymentDueDay) === 2 ? 'nd' : parseInt(paymentDueDay) === 3 ? 'rd' : 'th'} of month`} />
              <Row label="Notice period" value={`${noticePeriod} days`} />
              <Row label="Pets" value={petAllowed ? 'Allowed' : 'Not allowed'} />
              <Row label="Subletting" value={sublettingAllowed ? 'Allowed' : 'Not allowed'} />
              {specialConditions && <Row label="Special conditions" value={specialConditions} />}
            </Section>
          </div>
          ...
          <div className="mt-8 flex justify-between">
            <button onClick={() => setStep(2)} className="rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-medium text-slate-600 transition hover:bg-slate-50">
              ← Back
            </button>
            <button
              disabled={submitting}
