...
function daysLate(payment: Payment): number | null {
  ...
  const diff = new Date(payment.paid_date).getTime() - new Date(payment.due_date).getTime()
...
export default async function TenantPage({ params }: { params: { id: string } }) {
  ...
  if ((tenant.properties as { owner_id: string }).owner_id !== user.id) notFound()
        ...
        <div className="card mb-6 p-6">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <ScoreStat
              ...
              colour={risk.breakdown.streak > 0 ? 'text-emerald-600' : 'text-slate-400'}
            />
          </div>
        </div>
                ...
                return (
                  <div key={p.id} className="grid grid-cols-4 items-center px-6 py-3.5">
                    ...
                    <span className="text-sm text-slate-700">
                      ...
                      {late !== null && (
                        <span className="ml-1 text-xs text-amber-500">+{late}d</span>
                      )}
                    </span>
