...
export function ApplicationForm({ propertyId }: { propertyId: string }) {
  ...
  const ratioResult =
    rentCents > 0 && incomeCents > 0
      ? calcRatioFlag(rentCents, incomeCents)
      : null;
  ...
  const step2Valid = parseFloat(form.monthly_income || "0") > 0;
    ...
    try {
      const res = await fetch("/api/applications", {
        ...
        body: JSON.stringify({
          ...
          monthly_income_cents: Math.round(
            parseFloat(form.monthly_income) * 100,
          ),
    ...
    return (
      <div className="card p-10 text-center">
        ...
        <p className="mt-2 text-sm text-slate-500">
          Thank you, {form.full_name.split(" ")[0]}. The landlord will review
          your application and get in touch via {form.email}.
        </p>
      </div>
    );
  }
        ...
        {steps.map((label, i) => {
          ...
          return (
            <div key={n} className="flex items-center gap-2">
              {i > 0 && (
                <div
                  className={`h-px w-8 ${complete ? "bg-slate-900" : "bg-slate-200"}`}
                />
              )}
        ...
        <div className="card p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {idResult && (
                <div
                  className={`mt-2 rounded-lg p-3 text-sm ${idResult.valid ? "bg-emerald-50" : "bg-red-50"}`}
                >
                  {idResult.valid ? (
                    <div className="space-y-0.5 text-emerald-700">
                      <p className="font-medium">✓ Valid ID number</p>
                      <p>
                        DOB: {idResult.dob} · Age: {idResult.ageInYears} ·{" "}
                        {idResult.gender} ·{" "}
                        {idResult.citizenType === "citizen"
                          ? "SA Citizen"
                          : "Permanent Resident"}
                      </p>
                    </div>
            ...
            {ratioResult && (
              <div
                className={`rounded-lg border p-3 ${
                  ratioResult.flag === "green"
                    ? "border-emerald-200 bg-emerald-50"
                    : ratioResult.flag === "amber"
            ...
            <div>
              ...
              >
                ...
                <p className="mt-0.5 text-xs text-slate-400">
                  PDF only · Max 10 MB · Digital statements
                </p>
                <input
                  ref={fileRef}
                  type="file"
              ...
              {/* Upload progress */}
              {form.bankFilename &&
                (form.bankUploading || form.bankUploadProgress > 0) && (
                  <div className="mt-3">
              ...
              {form.bankParseError && (
                <p className="mt-2 rounded-lg bg-amber-50 p-3 text-sm text-amber-700">
                  {form.bankParseError} (statement uploaded but analysis
                  unavailable)
                </p>
              )}
              ...
              {/* Parsed summary */}
              {form.bankAnalysis && (
                <BankSummaryCard analysis={form.bankAnalysis} />
              )}
            </div>
          </div>
          ...
          <div className="card p-6">
            ...
            <dl className="grid grid-cols-2 gap-3 text-sm">
              ...
              <ReviewRow
                ...
                value={formatRand(
                  Math.round(parseFloat(form.monthly_income || "0") * 100),
                )}
              />
              {requestedRent && (
                <ReviewRow
    ...
    <div className="mt-3 rounded-lg border border-slate-200 bg-white p-4">
      <div className="mb-3 flex items-center justify-between">
        <p className="text-sm font-semibold text-slate-900">
          {analysis.bank
            ? `${analysis.bank} Statement Parsed`
            : "Statement Parsed"}
        </p>
      ...
      <div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
        <Stat
          label="Avg income/mo"
          value={
            analysis.avgMonthlyIncome > 0
              ? formatRand(analysis.avgMonthlyIncome)
              : "—"
          }
        />
        <Stat
          ...
          value={
            analysis.avgMonthlyExpenses > 0
              ? formatRand(analysis.avgMonthlyExpenses)
              : "—"
          }
        />
        <Stat
          label="Avg balance"
          value={
            analysis.avgMonthlyBalance > 0
              ? formatRand(analysis.avgMonthlyBalance)
              : "—"
          }
        />
        <Stat
          label="Salary months"
          value={`${analysis.salaryMonths} / ${analysis.totalMonthsAnalyzed}`}
        />
      </div>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {analysis.bouncedDos.length > 0 && (
            <span className="rounded-full bg-red-100 px-2.5 py-0.5 text-xs font-medium text-red-700">
              {analysis.bouncedDos.length} bounced debit order
              {analysis.bouncedDos.length > 1 ? "s" : ""}
            </span>
          )}
          {analysis.gamblingTransactions.length > 0 && (
            <span className="rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-medium text-amber-700">
              {analysis.gamblingTransactions.length} gambling transaction
              {analysis.gamblingTransactions.length > 1 ? "s" : ""}
            </span>
          )}
        </div>
      )}
