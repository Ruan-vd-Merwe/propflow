...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const {
      property_id, component_type, name, installed_date,
      lifespan_min_years, lifespan_max_years,
      brand, model_number, notes, last_serviced_date,
    } = body

    if (!property_id || !component_type || !name || !installed_date) {
      return NextResponse.json(
        { error: 'property_id, component_type, name and installed_date are required' },
        { status: 400 }
      )
    }
