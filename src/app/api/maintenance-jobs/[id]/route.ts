  ...
  try {
    ...
    if (action === "send_email") {
      ...
      const { data: job } = await supabase
        .from("maintenance_jobs")
        .select(
          "title, properties!inner(name, profiles!inner(full_name, email))",
        )
        .eq("id", params.id)
        .single();
      if (!job)
      ...
      if (process.env.RESEND_API_KEY) {
        ...
        const from =
          ...
          "PropTrust <notifications@proptrust.co.za>";
...
<div style="border:1px solid #e2e8f0;border-top:none;padding:24px;border-radius:0 0 8px 8px;">
  ...
  <p style="font-size:12px;color:#94a3b8;">
    ...
    Reply directly to this email or contact ${landlord.full_name} at ${landlord.email}
        ...
        console.log(
          `[maintenance-jobs DEV] Email to ${contractor_email}: ${job.title}`,
        );
