...
export async function POST(req: NextRequest) {
  ...
  const rangeStart = new Date(today); rangeStart.setDate(today.getDate() - 14)
  const rangeEnd   = new Date(today); rangeEnd.setDate(today.getDate() + 3)
  ...
  const results: { paymentId: string; type: string; sent: boolean; error?: string }[] = []
  ...
  for (const payment of payments ?? []) {
    ...
    await supabase.from('communications_log').insert({
      ...
      metadata:  { payment_id: payment.id, due_date: payment.due_date, days_from_due: daysFromDue },
    })
