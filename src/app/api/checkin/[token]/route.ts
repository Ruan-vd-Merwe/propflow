...
export async function POST(
  ...
  try {
    ...
    const { unit_working, maintenance_needed, maintenance_details, flag_text } =
      body as {
    ...
    if (
      typeof unit_working !== "boolean" ||
      typeof maintenance_needed !== "boolean"
    ) {
      return NextResponse.json(
        { error: "unit_working and maintenance_needed are required booleans" },
        { status: 400 },
      );
    }
    ...
    const { error } = await supabase
      ...
      .update({
        unit_working,
        maintenance_needed,
        maintenance_details: maintenance_needed
          ? maintenance_details?.trim() || null
          : null,
