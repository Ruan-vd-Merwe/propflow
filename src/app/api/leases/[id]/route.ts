...
export async function PATCH(
  ...
  if (fetchErr || !current) return NextResponse.json({ error: 'Lease not found' }, { status: 404 })
  if (current.landlord_id !== user.id) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
