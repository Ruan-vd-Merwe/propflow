...
import { calcCreditScore, calcRatioFlag, detectFraudFlags } from '@/lib/credit-score'
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── ID verification ──────────────────────────────────────────────────────
    const idVerification = id_number ? validateSAId(id_number) : {
      valid: false, checksumValid: false, dob: null, gender: null,
      citizenType: null, ageInYears: null, errors: ['No ID number provided'],
    }

    // ── Salary-to-rent ratio ─────────────────────────────────────────────────
    const emptyAnalysis: BankStatementAnalysis = {
      bank: null, accountNumber: null,
      avgMonthlyIncome: 0, avgMonthlyExpenses: 0, avgMonthlyBalance: 0,
      salaryMonths: 0, totalMonthsAnalyzed: 0,
      bouncedDos: [], gamblingTransactions: [], rentalPayments: [],
      largeCashDeposits: [], monthlyBreakdowns: [], parseWarnings: [], rawTextLength: 0,
    }
    ...
    const { flag: ratio_flag, percent: ratio_percent } =
      rent > 0 && income > 0 ? calcRatioFlag(rent, income) : { flag: null, percent: null }
    ...
    // ── Credit score ─────────────────────────────────────────────────────────
    const { score: credit_score, breakdown: credit_score_breakdown } = calcCreditScore({
      bankAnalysis:    analysis,
      rentCents:       rent,
      incomeCents:     income,
      idVerification,
