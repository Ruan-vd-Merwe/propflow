...
const SECTIONS = [
  ...
  },
  {
    id:    'deposits',
    title: 'Deposits',
    intro: 'The Rental Housing Act places specific obligations on landlords regarding the handling of tenant deposits.',
    points: [
      {
  ...
  },
  {
    ...
    intro: 'The notice period required to end a lease depends on whether the lease is fixed-term or month-to-month.',
    points: [
      {
  ...
  },
  {
    ...
    intro: 'Both landlords and tenants have obligations regarding the condition of the property.',
    points: [
      {
  ...
  },
  {
    id:    'eviction',
    title: 'Eviction',
    intro: 'Eviction in South Africa is governed by the Prevention of Illegal Eviction from and Unlawful Occupation of Land Act 19 of 1998 (PIE Act). A landlord cannot evict a tenant without a court order.',
    points: [
      {
  ...
  },
  {
    ...
    intro: 'The Consumer Protection Act 68 of 2008 applies to residential lease agreements where the landlord is acting in the ordinary course of business (e.g., professional landlords and property managers). It provides tenants with additional protections.',
    points: [
      {
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-900 antialiased">
      ...
      <div className="border-b border-amber-200 bg-amber-50 px-6 py-4">
        <div className="mx-auto max-w-7xl">
          <p className="text-xs leading-relaxed text-amber-800">
            <span className="font-bold">Note: </span>{DISCLAIMER}
          </p>
        </div>
      </div>
            ...
            <div className="space-y-16">
              {SECTIONS.map(section => (
                <div key={section.id} id={section.id}>
                  <h2 className="mb-4 text-2xl font-extrabold text-[#0f172a]">{section.title}</h2>
                  <p className="mb-8 text-base leading-relaxed text-slate-500">{section.intro}</p>

                  <div className="space-y-6">
                    {section.points.map(point => (
                      <div key={point.heading} className="rounded-xl border border-[#e2e8f0] bg-[#f8fafc] p-6">
                        <h3 className="mb-2 font-bold text-[#0f172a]">{point.heading}</h3>
              ...
              <div className="rounded-2xl border border-blue-100 bg-blue-50 p-8">
                <h2 className="mb-3 text-xl font-extrabold text-[#0f172a]">Need to resolve a dispute?</h2>
                ...
                <p className="text-sm font-semibold text-[#1e40af]">Western Cape: 021 483 5158</p>
                <p className="text-sm text-[#1e40af]">Gauteng: 011 355 4000</p>
                <p className="mt-1 text-xs text-slate-400">Contact your provincial government for local tribunal details.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="bg-[#f8fafc] px-6 py-20">
        <div className="mx-auto max-w-7xl">
          <h2 className="mb-8 text-xl font-extrabold text-[#0f172a]">Related guides</h2>
          <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
            {[
              { label: 'Tenant Screening Guide', href: '/resources/screening', desc: 'How to review applicants before signing a lease.' },
              { label: 'FAQ',                    href: '/resources/faq',       desc: 'Common questions about PropTrust and the rental process.' },
              { label: 'Features overview',      href: '/features',            desc: 'See how PropTrust supports a compliant rental workflow.' },
            ].map(r => (
