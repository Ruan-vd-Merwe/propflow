...
const FEATURES = [
  ...
  },
  {
    ...
    perks: ['Tenant portal submission', 'Status tracking', 'Photo uploads', 'Resolution history'],
  ...
  },
  {
    ...
    perks: ['Multi-property overview', 'Tenant status at a glance', 'Outstanding task list', 'Lease expiry tracking'],
  ...
  },
  {
    ...
    perks: ['Verified tenant profiles', 'Budget and area filters', 'Direct contact without agents', 'Available on Professional+ plans'],
  ...
  },
  {
    ...
    perks: ['Levy tracking', 'Meeting minute storage', 'Notice management', 'Available on Professional+ plans'],
    icon:  <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>,
  },
]

// ── Feature Wheel ─────────────────────────────────────────────────────────────

const WHEEL_COLORS = ['#1e3a8a','#1e40af','#1d4ed8','#2563eb','#3b82f6','#60a5fa','#93c5fd','#bfdbfe']

function FeatureWheel({ active, setActive }: { active: number | null; setActive: (i: number | null) => void }) {
  const cx = 160, cy = 160, R = 128, r = 46
  ...
  function segPath(i: number) {
    ...
    const g = 0.045
    const x0 = cx + R * Math.cos(a0 + g), y0 = cy + R * Math.sin(a0 + g)
    const x1 = cx + R * Math.cos(a1 - g), y1 = cy + R * Math.sin(a1 - g)
    const x2 = cx + r * Math.cos(a1 - g), y2 = cy + r * Math.sin(a1 - g)
    const x3 = cx + r * Math.cos(a0 + g), y3 = cy + r * Math.sin(a0 + g)
  ...
  return (
    <svg viewBox="0 0 320 320" className="w-full max-w-[300px] select-none" aria-hidden="true">
      {FEATURES.map((f, i) => {
        ...
        return (
          ...
          >
            <path
              ...
              style={{ transition: 'opacity .15s, transform .15s', transformOrigin: `${cx}px ${cy}px`, transform: isActive ? 'scale(1.04)' : 'scale(1)' }}
            />
            <text x={pos.x} y={pos.y - 5} textAnchor="middle" dominantBaseline="middle"
              fill={i >= 6 ? '#1e3a8a' : 'white'} fontSize="7" fontWeight="600">{line1}</text>
            {line2 && (
              <text x={pos.x} y={pos.y + 7} textAnchor="middle" dominantBaseline="middle"
                fill={i >= 6 ? '#1e3a8a' : 'white'} fontSize="7" fontWeight="600">{line2}</text>
            )}
          </g>
        )
      })}
      <circle cx={cx} cy={cy} r={r - 5} fill="#0f172a" />
      <text x={cx} y={cy - 6} textAnchor="middle" fill="white" fontSize="14" fontWeight="800">PT</text>
    ...
    <div className="min-h-screen bg-white font-sans text-slate-900 antialiased">
      ...
      <section className="bg-[#0f172a] px-6 pb-20 pt-20 text-center md:pt-28">
        <div className="mx-auto max-w-3xl">
          <p className="mb-4 text-xs font-bold uppercase tracking-widest text-blue-400">Platform features</p>
                ...
                >
                  <div className="flex items-start gap-4">
                    <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl transition-colors ${
                      i === highlighted ? 'bg-[#1e40af] text-white' : 'bg-white text-[#1e40af]'
                    }`}>
                      {f.icon}
                    </div>
                    <div>
      ...
      <section className="bg-[#f8fafc] px-6 py-24">
        <div className="mx-auto max-w-7xl space-y-24">
          {FEATURES.filter((_, i) => i < 4).map((f, i) => (
            <div key={f.id} id={f.id + '-detail'} className={`grid items-center gap-12 lg:grid-cols-2 ${i % 2 === 1 ? 'lg:grid-flow-dense' : ''}`}>
              ...
              <div className={i % 2 === 1 ? 'lg:col-start-2' : ''}>
                ...
                <h2 className="mb-4 text-2xl font-extrabold tracking-tight text-[#0f172a] md:text-3xl">{f.title}</h2>
                <p className="mb-6 text-base leading-relaxed text-slate-500">{f.body}</p>
              ...
              <div className={i % 2 === 1 ? 'lg:col-start-1 lg:row-start-1' : ''}>
                <div className="rounded-2xl border border-[#e2e8f0] bg-white p-6 shadow-sm">
                  <div className="mb-3 flex items-center gap-2">
                    ...
                    <span className="text-xs font-semibold text-slate-400">{f.title}</span>
                  </div>
      ...
      <section className="bg-white px-6 py-24 lg:hidden">
        <div className="mx-auto max-w-7xl">
          <h2 className="mb-10 text-center text-2xl font-extrabold text-[#0f172a]">All features</h2>
          <div className="grid gap-4 sm:grid-cols-2">
            {FEATURES.map(f => (
              <div key={f.id} className="rounded-xl border border-[#e2e8f0] bg-[#f8fafc] p-5">
                <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-blue-50 text-[#1e40af]">{f.icon}</div>
                <p className="font-bold text-[#0f172a]">{f.title}</p>
      ...
      <section className="bg-[#1e40af] px-6 py-24 text-center">
        <div className="mx-auto max-w-xl">
          <h2 className="mb-5 text-3xl font-extrabold text-white">Start managing rentals properly</h2>
          <p className="mb-10 text-blue-100">All features available free for 30 days. No credit card required.</p>
          <div className="flex flex-wrap justify-center gap-4">
            ...
            <Link href="/register" className="rounded-full border-2 border-white/40 px-8 py-4 text-base font-bold text-white transition hover:border-white hover:bg-white/10">
              Book a demo
            </Link>
          </div>
        </div>
