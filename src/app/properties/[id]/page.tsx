  ...
  return (
    <div className="card flex flex-col gap-4 p-5">
      ...
      <dl className="grid grid-cols-2 gap-2 text-sm">
        ...
        <Chip label="Looking in">{tp.looking_in_area ?? "—"}</Chip>
      </dl>
...
function ListingIncompleteNotice({
  missingFields,
}: {
  ...
  return (
    <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 px-5 py-4">
      ...
      <p className="mt-1 text-xs text-amber-700">
        Missing: {missingFields.join(", ")}. Matches using partial data may be
        less accurate.
      </p>
    </div>
  );
            ...
            {matchedTenants.length === 0 ? (
              <div className="card p-12 text-center text-slate-500">
                <p className="text-base font-semibold text-slate-700">
                  No tenant profiles yet
                </p>
                <p className="mt-2 text-sm">
                  Tenants who register on PropTrust and mark themselves as
                  actively looking will appear here, ranked by match score.
                </p>
              </div>
            ) : (
