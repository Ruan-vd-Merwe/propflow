    ...
    <div>
      <div className="mb-2 flex items-center justify-between">
        <p className="text-sm font-medium text-slate-700">
          Photos <span className="text-slate-400">({used}/{totalSlots})</span>
        </p>
        {pending.length > 0 && (
          <button
      ...
      <div className="flex flex-wrap gap-2">
        {/* Uploaded photos */}
        {photos.map((url, i) => (
          <div key={url} className="group relative h-24 w-24 overflow-hidden rounded-xl border border-slate-200">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={url} alt={`Photo ${i + 1}`} className="h-full w-full object-cover" />
            <button
              onClick={() => { removeUploaded(url); saveRemoved() }}
        ...
        {pending.map((file, i) => (
          <div key={i} className="group relative h-24 w-24 overflow-hidden rounded-xl border-2 border-dashed border-slate-400">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={URL.createObjectURL(file)} alt="" className="h-full w-full object-cover opacity-70" />
            <button
