...
export default function NewsAdminPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      const ok = prof?.is_landlord || (adminEmail && prof?.email === adminEmail)
  ...
  async function handleFetch() {
    ...
    if (data) {
      showToast(`Fetched ${data.fetched} articles, stored ${data.stored} new, summarized ${data.summarized}.`)
      await loadDigest()
    }
  }
  ...
  async function handleSend() {
    ...
    if (data) {
      showToast(`Sent to ${data.sent} subscribers. ${data.failed > 0 ? `${data.failed} failed.` : ''}`)
      await loadDigest()
    }
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {toast && (
        <div className={`fixed bottom-6 left-1/2 z-50 -translate-x-1/2 rounded-xl px-5 py-3 text-sm font-medium text-white shadow-lg ${toast.ok ? 'bg-slate-900' : 'bg-red-600'}`}>
          {toast.msg}
        </div>
      )}
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Property News Digest</h1>
            <p className="mt-1 text-sm text-slate-500">
              Manage the weekly SA property market newsletter
              {subCount !== null && <span> · {subCount} active subscriber{subCount !== 1 ? 's' : ''}</span>}
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            {digest && (
              <div className="rounded-2xl border border-slate-200 bg-white p-5">
                <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-slate-400">Latest digest</p>
                <p className="text-sm font-medium text-slate-900">{digest.subject ?? 'Untitled'}</p>
                <p className="mt-0.5 text-xs text-slate-400">
                  {digest.week_start_date} – {digest.week_end_date}
                </p>
                <div className="mt-2 flex items-center gap-2">
                  <StatusBadge status={digest.status} />
                  <span className="text-xs text-slate-400">{digest.articles_count} articles</span>
                </div>
                {digest.sent_at && (
                  <p className="mt-1 text-xs text-slate-400">
                    Sent {new Date(digest.sent_at).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short', year: 'numeric' })}
                  </p>
                )}
              </div>
            )}
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 space-y-3">
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-400 mb-4">Actions</p>
              ...
              >
                ...
                {busy('fetch') && <span className="h-4 w-4 animate-spin rounded-full border-2 border-slate-400 border-t-transparent" />}
              </button>
              ...
              >
                ...
                {busy('generate') && <span className="h-4 w-4 animate-spin rounded-full border-2 border-slate-400 border-t-transparent" />}
              </button>
              ...
              {digest?.status === 'draft' && (
                ...
                >
                  ...
                  {busy('approve') && <span className="h-4 w-4 animate-spin rounded-full border-2 border-blue-400 border-t-transparent" />}
                </button>
              )}
              ...
              {digest?.status === 'approved' && (
                ...
                >
                  ...
                  {busy('send') && <span className="h-4 w-4 animate-spin rounded-full border-2 border-white/50 border-t-white" />}
                </button>
              )}
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {digest?.html_content ? (
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center">
                <p className="text-sm text-slate-400">No digest generated yet.</p>
                <p className="mt-1 text-xs text-slate-300">Fetch news and generate a digest to see the preview.</p>
              </div>
            )}
                ...
                <div className="divide-y divide-slate-100">
                  {articles.map((a, i) => (
                    <div key={a.id ?? i} className="flex items-start gap-3 px-5 py-3">
                      <span className="mt-0.5 w-5 shrink-0 text-xs font-bold text-slate-400">{i + 1}</span>
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-0.5 flex flex-wrap gap-2 text-xs text-slate-400">
                          ...
                          {a.published_at && (
                            <span>{new Date(a.published_at).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short' })}</span>
                          )}
                        </div>
                      </div>
                      <span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-bold ${
                        a.relevance_score >= 75 ? 'bg-green-100 text-green-700' :
                        a.relevance_score >= 50 ? 'bg-amber-100 text-amber-700' :
