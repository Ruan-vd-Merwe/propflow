...
export default async function ServicesPage({
  ...
  if (searchParams.cat)
    provQuery = provQuery.eq("category_id", searchParams.cat);
  if (searchParams.province)
    provQuery = provQuery.eq("province", searchParams.province);
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Book local services for your tenants. Providers are notified via
            WhatsApp.
          </p>
        </div>
        ...
        <div className="mb-8 grid gap-3 sm:grid-cols-2 md:grid-cols-3">
          {categories.map((cat) => {
            ...
            const providerCount = (provs ?? []).filter(
