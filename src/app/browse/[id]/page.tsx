...
export default async function BrowsePropertyPage({
  ...
  let scoreResult: Awaited<ReturnType<typeof tenant_interest_match_model>> | null = null
  ...
  if (property.suburb || property.province) {
    const { data: simRaw } = await supabase
      ...
      .eq('is_listed', true)
      .neq('id', property.id)
      .eq(property.suburb ? 'suburb' : 'province', (property.suburb ?? property.province) as string)
      .limit(3)
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        ...
        <nav className="mb-6 flex items-center gap-2 text-sm text-slate-500">
          ...
          <span>/</span>
          {property.suburb && <><Link href={`/browse?suburb=${encodeURIComponent(property.suburb)}`} className="hover:text-slate-900">{property.suburb}</Link><span>/</span></>}
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_360px]">
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              {(property.suburb || property.province) && (
                <div className="mt-1.5 flex items-center gap-1.5 text-sm text-slate-500">
                  <IconMapPin />
                  <span>{property.suburb}{property.province ? `, ${property.province}` : ''}</span>
                </div>
              )}
            </div>
            ...
            <div className="rounded-2xl bg-white p-5 shadow-sm">
              <h2 className="mb-4 text-sm font-semibold uppercase tracking-wide text-slate-400">Property details</h2>
              <table className="w-full text-sm">
                <tbody className="divide-y divide-slate-100">
                  {[
                    { label: 'Property type',  value: property.property_type ? property.property_type.charAt(0).toUpperCase() + property.property_type.slice(1) : '—' },
                    { label: 'Bedrooms',       value: property.bedrooms != null ? (property.bedrooms === 0 ? 'Studio' : String(property.bedrooms)) : '—' },
                    { label: 'Floor size',     value: property.floor_size_m2 ? `${property.floor_size_m2} m²` : '—' },
                    { label: 'Available from', value: property.available_from ? fmtDate(property.available_from) : '—' },
                    { label: 'Pet friendly',   value: property.pets_allowed ? 'Yes' : 'No' },
                    { label: 'Parking',        value: property.parking_available ? 'Included' : 'Not included' },
                    { label: 'Fibre',          value: property.fibre_available ? 'Available' : 'Not listed' },
                    { label: 'Deposit',        value: deposit ? fmtRand(deposit) : '—' },
                  ].map(row => (
                    <tr key={row.label}>
                      <td className="py-2.5 pr-4 text-slate-500">{row.label}</td>
                      <td className="py-2.5 font-medium text-slate-900">{row.value}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            {(property.property_tags?.length > 0 || property.area_tags?.length > 0 || property.lifestyle_tags?.length > 0) && (
              <div className="rounded-2xl bg-white p-5 shadow-sm">
                <h2 className="mb-4 text-sm font-semibold uppercase tracking-wide text-slate-400">Features &amp; area</h2>
                <div className="flex flex-wrap gap-2">
                  {[...property.property_tags ?? [], ...property.area_tags ?? [], ...property.lifestyle_tags ?? []].map(tag => (
                    <span key={tag} className="rounded-full border border-slate-200 px-3 py-1 text-xs font-medium capitalize text-slate-600">
                      {tag.replace(/_/g, ' ')}
                    </span>
                  ))}
                </div>
          ...
          <div className="space-y-4 lg:sticky lg:top-6 lg:self-start">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              {isTenant && hasTenantProfile ? (
                ...
                <>
                  <p className="mb-3 text-center text-sm text-slate-500">You are viewing as a landlord.</p>
                  <Link
                    href="/settings"
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5">
              <div className="mb-3 flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-900">Verified PropTrust landlord</p>
                  {memberSince && (
              ...
              {ownerListingCount != null && ownerListingCount > 0 && (
                <p className="text-xs text-slate-500">
                  {ownerListingCount} active {ownerListingCount === 1 ? 'listing' : 'listings'}
                </p>
              )}
              <p className="mt-2 text-xs text-slate-400">
                Landlord contact details are shared only after your application is accepted.
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 lg:grid lg:grid-cols-3 lg:overflow-visible">
              {similar.map(p => (
                ...
                >
                  {p.photos?.length > 0 ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={p.photos[0]} alt={p.name} className="h-36 w-full object-cover" />
                  ) : (
                  ...
                  <div className="p-3">
                    <p className="text-sm font-semibold text-slate-900 group-hover:text-blue-700">{p.name}</p>
                    <p className="mt-0.5 text-xs text-slate-400">{p.suburb}</p>
                    {p.asking_rent && (
                      <p className="mt-1.5 text-sm font-bold text-slate-900">
                        {fmtRand(p.asking_rent)}<span className="text-xs font-normal text-slate-400">/mo</span>
                      </p>
                    )}
                  </div>
                </Link>
