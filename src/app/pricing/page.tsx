...
const TIERS: Tier[] = [
  {
    name: 'Starter', monthly: 99, annual: 990,
    sub: 'For landlords with up to 3 properties.',
    includes: ['Unlimited tenant records', 'Tenant screening workflow', 'Rent tracking', 'Email reminders', 'Tenant portal', 'Document storage'],
    excludes: ['WhatsApp notifications', 'Body corporate tools', 'Advanced maintenance management'],
    cta: 'Start free trial', ctaHref: '/register',
  },
  {
    name: 'Professional', monthly: 299, annual: 2990,
    sub: 'For landlords and managers with up to 15 properties.',
    featured: true,
    includes: ['Everything in Starter', 'WhatsApp notifications', 'Body corporate tools', 'Maintenance management', 'Bank statement review', 'Tenant marketplace access', 'Priority support'],
    excludes: [],
    cta: 'Start free trial', ctaHref: '/register',
  },
  {
    name: 'Enterprise', monthly: 799, annual: 7990,
    sub: 'For larger portfolios and property teams.',
    includes: ['Everything in Professional', 'Unlimited properties', 'Dedicated account support', 'Custom integrations', 'API access', 'White-label options', 'SA document templates'],
    excludes: [],
...
function Tick({ on, featured }: { on: boolean; featured?: boolean }) {
  if (on) {
    return (
      <div className={`mx-auto flex h-5 w-5 items-center justify-center rounded-full ${featured ? 'bg-blue-500' : 'bg-green-500'}`}>
...
function PlanCheck({ included, featured }: { included: boolean; featured?: boolean }) {
  if (!included) {
    return (
  ...
  return (
    <div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${featured ? 'bg-blue-500' : 'bg-green-500'}`}>
      <svg className="h-3 w-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
      </svg>
    </div>
    ...
    <div className="min-h-screen bg-white font-sans text-slate-900 antialiased">
      ...
      <section className="bg-[#0f172a] px-6 pb-20 pt-20 text-center md:pt-28">
        <div className="mx-auto max-w-2xl">
          <p className="mb-4 text-xs font-bold uppercase tracking-widest text-blue-400">Pricing</p>
          ...
          <p className="text-lg text-slate-300">
            No agent commission. No confusing add-ons. Practical tools at an honest price.
          </p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mb-12 flex justify-center">
            <div className="flex items-center gap-4 rounded-xl border border-[#e2e8f0] bg-white p-1.5 shadow-sm">
              ...
              >
                Annual
                <span className={`rounded-full px-2 py-0.5 text-[10px] font-extrabold ${annual ? 'bg-green-500 text-white' : 'bg-green-100 text-green-700'}`}>
                  2 months free
                </span>
              </button>
            </div>
            ...
            {TIERS.map(tier => {
              ...
              return (
                ...
                >
                  ...
                  <p className={`mb-4 text-xs font-bold uppercase tracking-wider ${tier.featured ? 'text-blue-300' : 'text-slate-400'}`}>
                    {tier.name}
                  </p>

                  <div className="mb-2 flex items-end gap-1">
                    <span className={`text-4xl font-extrabold tabular-nums ${tier.featured ? 'text-white' : 'text-[#0f172a]'}`}>
                      R{price}
                    </span>
                    <span className={`mb-1 text-sm ${tier.featured ? 'text-blue-300' : 'text-slate-400'}`}>/month</span>
                  </div>
                  ...
                  <p className={`mb-8 text-sm ${tier.featured ? 'text-blue-200' : 'text-slate-500'}`}>
                    {tier.sub}
                  </p>
                  ...
                  <ul className="mb-8 flex-1 space-y-3">
                    {tier.includes.map(f => (
                      <li key={f} className="flex items-start gap-2.5">
                        ...
                        <span className={`text-sm ${tier.featured ? 'text-blue-50' : 'text-slate-700'}`}>{f}</span>
                      </li>
                    ))}
                    {tier.excludes.map(f => (
                      <li key={f} className="flex items-start gap-2.5">
      ...
      <section className="bg-white px-6 py-20">
        <div className="mx-auto max-w-5xl">
          <h2 className="mb-10 text-center text-2xl font-extrabold text-[#0f172a]">Feature comparison</h2>

          <div className="overflow-x-auto rounded-2xl border border-[#e2e8f0]">
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#e2e8f0] bg-[#f8fafc]">
                  <th className="px-6 py-4 text-left text-sm font-bold text-slate-500">Feature</th>
                  {TIERS.map(t => (
                    <th key={t.name} className="px-6 py-4 text-center text-sm font-bold text-[#0f172a]">{t.name}</th>
                  ))}
                </tr>
              </thead>
      ...
      <section className="bg-[#f8fafc] px-6 py-20">
        <div className="mx-auto max-w-2xl">
          <h2 className="mb-10 text-center text-2xl font-extrabold text-[#0f172a]">Pricing questions</h2>

          <div className="space-y-3">
            {FAQS.map((faq, i) => (
              <div key={i} className="rounded-xl border border-[#e2e8f0] bg-white">
                <button
      ...
      <section className="bg-[#1e40af] px-6 py-24 text-center">
        <div className="mx-auto max-w-xl">
          ...
          <p className="mb-10 text-blue-100">No credit card required. Cancel anytime.</p>
