...
export function EditPreferencesPanel({
  tenantProfile,
  userId,
  ...
  const [budgetMin, setBudgetMin] = useState(
    Math.round((tenantProfile.budget_min ?? 300000) / 100),
  );
  const [budgetMax, setBudgetMax] = useState(
    Math.round((tenantProfile.budget_max ?? 1500000) / 100),
  );
  ...
  return (
    <>
      ...
      {/* ── Slide-over panel ───────────────────────────────────────────────── */}
      <div
        className={`fixed inset-y-0 right-0 z-50 flex w-full flex-col bg-white shadow-2xl transition-transform duration-300 ease-out sm:max-w-md ${
