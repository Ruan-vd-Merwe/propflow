  ...
  function toggleArea(val: string) {
    setPrefs((p) => {
      ...
      return {
        ...p,
        area_interests: next,
        must_haves:  p.must_haves.filter((v) => next.includes(v) || p.property_interests.includes(v) || p.lifestyle_interests.includes(v)),
        dealbreakers: p.dealbreakers.filter((v) => next.includes(v) || p.property_interests.includes(v) || p.lifestyle_interests.includes(v)),
      }
    })
  }
  ...
  function toggleLifestyle(val: string) {
    setPrefs((p) => {
      ...
      return {
        ...
        must_haves:  p.must_haves.filter((v) => next.includes(v) || p.property_interests.includes(v) || p.area_interests.includes(v)),
        dealbreakers: p.dealbreakers.filter((v) => next.includes(v) || p.property_interests.includes(v) || p.area_interests.includes(v)),
      }
    })
  }

  function toggleMustHave(val: string) {
    setPrefs((p) => ({ ...p, must_haves: toggle(p.must_haves, val), dealbreakers: p.dealbreakers.filter((v) => v !== val) }))
  }

  function toggleDealbreaker(val: string) {
    setPrefs((p) => ({ ...p, dealbreakers: toggle(p.dealbreakers, val), must_haves: p.must_haves.filter((v) => v !== val) }))
  }
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
        ...
        <div className="mb-6 flex items-center gap-3">
          <Link href="/tenant/profile" className="text-sm text-slate-400 hover:text-slate-700">
            Profile
          </Link>
          <span className="text-slate-300">/</span>
          <span className="text-sm font-medium text-slate-700">Search preferences</span>
        </div>

        <div className="mb-6">
          <h1 className="text-2xl font-bold text-slate-900">Search preferences</h1>
          <p className="mt-1 text-sm text-slate-500">
            Tell us what matters to you — PropTrust uses this to rank properties by how well they fit your life.
          </p>
        </div>
          ...
          <Section title="The basics">
            ...
            <div className="mb-5">
              ...
              <div className="flex flex-wrap gap-2">
                {BEDROOM_OPTIONS.map((o) => (
                  <button
                    ...
                    onClick={() =>
                      setPrefs((p) => ({
                        ...p,
                        desired_bedrooms: p.desired_bedrooms === o.value ? null : o.value,
                      }))
                    }
                ...
                <button
                  type="button"
                  onClick={() => setPrefs((p) => ({ ...p, desired_bedrooms: null }))}
                  className={`rounded-full border px-4 py-1.5 text-sm font-medium transition ${
            ...
            <div className="mb-5 flex flex-wrap gap-4">
              <label className="flex cursor-pointer items-center gap-3">
                <button
                  ...
                  onClick={() => setPrefs((p) => ({ ...p, has_car: !p.has_car }))}
                ...
                <span className="text-sm font-medium text-slate-700">I have a car</span>
              </label>
              ...
              <label className="flex cursor-pointer items-center gap-3">
                <button
                  ...
                  onClick={() => setPrefs((p) => ({ ...p, has_pets: !p.has_pets }))}
                  className={`relative inline-flex h-6 w-11 shrink-0 rounded-full border-2 border-transparent transition-colors focus:outline-none ${
                ...
                <span className="text-sm font-medium text-slate-700">I have pets</span>
              </label>
            </div>
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-slate-700">
                Work location
                <span className="ml-1 text-xs font-normal text-slate-400">(suburb or area — for commute scoring)</span>
              </label>
              <input
                type="text"
                placeholder="e.g. Sandton CBD, Stellenbosch, Remote"
                value={prefs.work_location}
                onChange={(e) => setPrefs((p) => ({ ...p, work_location: e.target.value }))}
