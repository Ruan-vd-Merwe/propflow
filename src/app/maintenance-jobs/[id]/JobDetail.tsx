...
const URGENCY_LABEL = { urgent: '🔴 Urgent', normal: '🟡 Normal', planned: '🟢 Planned' }
...
export function JobDetail({ job: initialJob, property, component, allComponents }: Props) {
  ...
  const [contractorName,  setContractorName]  = useState(job.contractor_name  ?? '')
  const [contractorEmail, setContractorEmail] = useState(job.contractor_email ?? '')
  ...
  async function handleGenerate() {
    ...
    setSuccess('')
    setError('')
    const json = await patch({ action: 'generate_description' }, 'Description generated!')
    if (json?.description) {
      setFinalDesc(json.description)
      setJob((j) => ({ ...j, generated_description: json.description, final_description: json.description }))
    }
    setBusy(null)
  }
  ...
  async function handleSummariseQuote() {
    ...
    setSuccess('')
    setError('')
    const cents = quoteAmount ? Math.round(parseFloat(quoteAmount) * 100) : null
    const json  = await patch({
    ...
    <div className="space-y-5">
      ...
      <div className="card p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex shrink-0 flex-col items-end gap-1">
            <span className={`rounded-full px-3 py-1 text-sm font-medium ${statusCfg.badge}`}>
              {statusCfg.label}
            </span>
            <span className="text-xs text-slate-400">{URGENCY_LABEL[job.urgency]}</span>
          </div>
        </div>
        ...
        {component && (
          <div className="mt-4 rounded-lg bg-slate-50 p-3">
            <div className="flex items-center gap-2">
              <p className="text-sm font-medium text-slate-700">Component: {component.name}</p>
              {(() => {
                const h = getComponentHealth(component.installed_date, component.lifespan_max_years)
                return (
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${componentStatusClass(h.status)}`}>
                    {h.label}
                  </span>
                )
              })()}
            ...
            <p className="mt-0.5 text-xs text-slate-400">
              Installed {component.installed_date} · Max lifespan {component.lifespan_max_years}y
            </p>
          </div>
        )}
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <div className="space-y-5">
          ...
          <div className="card p-6">
            <h2 className="mb-4 font-semibold text-slate-900">2. Send to Contractor</h2>
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-slate-700">Contractor email *</label>
                <input
                  type="email"
            ...
            <button
              ...
              disabled={busy === 'send_email' || !contractorEmail.trim() || !finalDesc.trim()}
        ...
        <div className="space-y-5">
          ...
          <div className="card p-6">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="font-semibold text-slate-900">3. Contractor Quote</h2>
              <button
          ...
          <div className="card p-6">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {['draft', 'sent', 'declined', 'completed'].includes(job.status) && (
                <button
            ...
            {/* Landlord notes */}
            <LandlordNotesForm jobId={job.id} initialNotes={job.landlord_notes ?? ''} />
          </div>
        </div>
      </div>
      ...
      {allComponents.length > 0 && (
        <div className="card p-6">
          <h2 className="mb-4 font-semibold text-slate-900">Other components at {property.name}</h2>
          <Link
...
function LandlordNotesForm({ jobId, initialNotes }: { jobId: string; initialNotes: string }) {
